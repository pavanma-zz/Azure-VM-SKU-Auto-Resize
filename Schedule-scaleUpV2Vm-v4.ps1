<# 
.SYNOPSIS 
  Connects to Azure and vertically scales up the VM based on schedule
 
.DESCRIPTION 
  This runbook connects to Azure and schedules runbook that scales up the VM  
...
# Register-AzureRmAutomationScheduledRunbook -AutomationAccountName 'pavanma-aa' -Name "scaleUpV2Vm-v4.ps1" -ScheduleName "test2pavan-auto-resize" -ResourceGroupName "TestRGForPavan"
    ...
    if (Register-AzureRmAutomationScheduledRunbook -Name "scaleUpV2Vm-v4.ps1" -ScheduleName $scheduleName  -AutomationAccountName $AutoAccName -ResourceGroupName $AutoAccNameRG -Parameters $params) {
